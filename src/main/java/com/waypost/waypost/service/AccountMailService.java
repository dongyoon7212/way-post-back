...
public class AccountMailService {
    ...
    public boolean sendAuthMail() {
        ...
        String toMailAddress = principalUser.getUser().getEmail();

        MimeMessage mimeMessage = javaMailSender.createMimeMessage();

        try {
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, false, "utf-8"); // 파일을 전송한다면 두번째 매개변수 true
            helper.setSubject("Way Post 회원가입 메일 인증"); //메일의 제목
            helper.setFrom(fromMailAddress);
            helper.setTo(toMailAddress);

            String authMailToken = jwtUtil.generateAuthMailToken(userId, toMailAddress);

            StringBuilder mailContent = new StringBuilder();
            mailContent.append("<div>");
            mailContent.append("<h1>회원가입 메일 인증</h1>");
